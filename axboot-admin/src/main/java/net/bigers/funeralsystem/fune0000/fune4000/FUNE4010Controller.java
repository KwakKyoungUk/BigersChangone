/*****************************************************************************
 * 프로그램명  : FUNE4010Controller.java
 * 설     명  : 생산관리 처리 컨트롤러
 * 참고  사항  : 없음
 *****************************************************************************
 * Date       Author  Version Description
 * ---------- ------- ------- -----------------------------------------------
 * 2017.11.06  KYM    1.0     초기작성
 *****************************************************************************/
...
import net.bigers.funeralsystem.fune0000.domain.production_stmt_bd_cost.ProductionStmtBdCostService;
import net.bigers.funeralsystem.fune0000.domain.user_mng_part.UserMngPartService;

@Controller
public class FUNE4010Controller extends BaseController{
	...
	@Inject
	private ProductionStmtBdCostService productionStmtBdCostService;

	/**
	 * 검색 콤보 파트 목록 취득
	 */
	@ApiOperation(value = "검색파트", notes = "검색파트")
	@RequestMapping(value="/FUNE4010/part", method={RequestMethod.POST}, produces=APPLICATION_JSON) // POST 방식을 사용하였는데, AXSearch 컴포넌트에서 AJAX 요청시 GET 방식으로 받아오지 않기에 POST방식으로 사용하였다.
	...
	@RequestMapping(value="/FUNE4010/productionStmt" , method= RequestMethod.GET, produces=APPLICATION_JSON )
	 ...
	 */
	@ApiOperation(value = "생산 전표 조회", notes = "생산 전표 조회")
	@RequestMapping(value="/FUNE4010/arrange" , method= RequestMethod.POST, produces=APPLICATION_JSON )
	public ApiResponse arrange(String partCode, @DateTimeFormat(pattern="yyyy-MM-dd") Date etDate) throws Exception{
		this.productionStmtBdCostService.arrangeStmt(partCode, etDate);
		return ok();
	}
}

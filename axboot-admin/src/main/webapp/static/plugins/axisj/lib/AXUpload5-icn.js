...
(function(){
	...
	SWFUpload.prototype.getFlashHTML = function (flashVersion) {
		...
		return ['<object id="', this.movieName, '" type="application/x-shockwave-flash" style="position:absolute;left:0px;top:0px;" data="', (this.support.imageResize ? this.settings.flash_url : this.settings.flash9_url), '" width="', this.settings.button_width, '" height="', this.settings.button_height, '" class="swfupload">',
...
var AXUpload5 = Class.create(AXJ, {
	...
	swfinit: function(reset){
		...
		po.push('<div style="position:relative;">');
		po.push('	<table style="table-layout:fixed;width:100%;"><tbody><tr><td id="'+cfg.targetID+'_AX_selectorTD">');
		po.push('	<button type="button" style="width:100%;" class="AXButton '+cfg.targetButtonClass+'" id="'+cfg.targetID+'_AX_selector"><span class="AXFileSelector" style="">'+(cfg.buttonTxt)+'</span></button>');
		po.push('	<span style="position:absolute;left:0px;top:0px;" id="'+cfg.targetID+'_AX_spanButtonPlaceholder" class="AXUpload5flashUploadButton"></span>');
		po.push('	</td>');
		
		if(cfg.isSingleUpload){
			//po.push('<td>');
			po.push('<div class="AXFileDisplay" id="'+cfg.targetID+'_AX_display"></div>');
			//po.push('</td>');
		}
		...
		var settings = {
			...
			button_placeholder_id : cfg.targetID + "_AX_spanButtonPlaceholder",
			button_width: btnW,
			button_height: btnH,
	...
	uploadQueue: function(fileUpload){
		...
		this.xhr.onload = function(e) {
			...
			if (res.result == "err" || res.error) {
				...
				//trace(res);
				if(!cfg.isSingleUpload) axdom("#" + itemID).fadeOut("slow");
				if(cfg.onError) cfg.onError.call(res, res.error.status, res.error.message);
				cancelUpload();
				return;
			}

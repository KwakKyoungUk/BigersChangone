...
//            var map_p = cfg.colHead._maps[mr].last();
        	var temp = cfg.colHead._maps[mr];

            var map_p;
            for(var idx = temp.length-1;idx>0; idx--){
            	map_p = temp[idx];
            	if(map_p){
            		break;
            	}
                ...
                setTimeout(function () {
                    if (
                        (
                            ...
                            e.keyCode == axf.Event.KEY_LEFT || e.keyCode == axf.Event.KEY_RIGHT
                        ) && (!e.altKey)
                    ) {
                        if (
                        		e.keyCode == axf.Event.KEY_RETURN || e.keyCode == axf.Event.KEY_TAB
                        		|| e.keyCode == axf.Event.KEY_UP || e.keyCode == axf.Event.KEY_DOWN
                        		|| e.keyCode == axf.Event.KEY_LEFT || e.keyCode == axf.Event.KEY_RIGHT
                        		) {
                        ...
                        else if (e.keyCode == axf.Event.KEY_LEFT || e.keyCode == axf.Event.KEY_RIGHT) {
                        	var new_c, ci, cl;
                            ci = 0, cl = cfg.colGroup.length;
                            if (e.keyCode == axf.Event.KEY_LEFT) {
                            	if(+c==0){
                            		if(ii != 0){
                            			ii--;
                            		}

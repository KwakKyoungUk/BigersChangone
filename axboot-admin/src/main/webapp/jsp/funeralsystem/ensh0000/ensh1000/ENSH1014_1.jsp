	...
	//						pay.amt, pay.vat
				...
				defaultFn: {
                	...
                	, fn: {
						getEnsext: function(){
							app.ajax({
	                            type: "GET",
	                            url: "/ENSH1014_1/ensext",
	                            data: "extDate=${param.extDate}&extSeq=${param.extSeq}"
	                        },
	                        function(res){
								if(res.error){
									alert(res.error.message);
								}else{
									fnObj.data.ensext = res.map.ensext;
								}
	                        });
						}
                	}
                ...
                eventFn: {
	            	...
	            	, savePaymentCash: function(gubun, data){
                		...
                		app.ajax({
                            type: "POST",
                            url: "/ENSH1014_1/payment?deadId=${param.deadId}&extDate=${param.extDate}&extSeq=${param.extSeq}",
...
// 	            	, cancel: function(kind, etDate, appNo, amount, vat){
            		, cancel: function(itemIdx){
	            		...
	            		if(!appNo || appNo == '' || appNo == 'undefined' || (item.paymentCard && item.paymentCard.notice == "수동입력")){
	            			...
	            			app.ajax({
	                            type: "POST",
	                            url: "/ENSH1014_1/payment/cancel?deadId=${param.deadId}&extDate=${param.extDate}&extSeq=${param.extSeq}&deadId="+item.deadId+"&tid="+item.tid+"&seq="+item.seq,
	                            data: Object.toJSON(payment)
	                        },
	                        function(res){
								if(res.error || res.status == "-500"){
...
//                 			console.log(data);
	            		...
	            		if(item.kind[0] == 'D'){
	            			...
	            			Pay.Cancel.cash(item.cashKind, item.cashAmt, item.vatAmt, item.cardNo, item.shopTs, item.etDate.date().print("yyyymmdd"), item.appNo, callback)
	            		}

// 	            		ezr.window.requestReturnCancelTax(kind, amount, etDate.date().print("yyyymmdd").substring(2), appNo, 0, , halbu);
                	}
                },
                modal:{
                    ...
                    bind: function(){
                        ...
                        target.setConfig({
                            ...
                            colGroup : [
                                ...
                                {key:"amount", label:"연장비용", width:"100", align:"center", formatter: "money"},
                                {key:"charge", label:"부과액", width:"100", align:"center", formatter: function(){
                    ...
                    bind: function(){
                        ...
                        target.setConfig({
                            ...
                            colGroup : [
                                ...
                                {key:"amount", label:"결제금액", width:"100", align:"center", formatter: function(){
...
//                                 		return '<button type="button" class="AXButton" onclick="fnObj.eventFn.cancel(\''+this.item.kind+'\', \''+this.item.etDate+'\', \''+this.item.appNo+'\', \''+(this.item.totAmt-this.item.dcAmt)+'\', \''+this.item.vatAmt+'\')">취소</button>'
                    ...
                    setPage: function(){
                    	app.ajax({
                            type: "GET",
                            url: "/ENSH1014_1/payment",
                            data: "deadId=${param.deadId}&partCode=81-001"
                        },
                        function(res){
                        	if(res.error){
                        		alert(res.error.message);
